const { EmbedBuilder } = require("discord.js");
const { User } = require("../database/mongo");

module.exports = {
    name: "convertpoint",
    description: "Convert points ke credits (1 point = 100 credits)",
    aliases: ["cp"],
    
    async execute(message, args, client) {
        const amount = parseInt(args[0]);
        
        if (isNaN(amount) || amount <= 0) {
            return message.reply("❌ Usage: `!convertpoint <jumlah_points>`\n\n💡 1 point = 100 credits\nContoh: `!convertpoint 10` → 1000 credits");
        }
        
        const creditsAmount = amount * 100;
        
        try {
            const user = await User.findOne({ userId: message.author.id });
            
            if (!user || user.points < amount) {
                return message.reply(`❌ Point tidak cukup! Punya ${user?.points || 0} points.`);
            }
            
            await User.updateOne(
                { userId: message.author.id },
                {
                    $inc: {
                        credits: creditsAmount,
                        points: -amount
                    }
                }
            );
            
            const embed = new EmbedBuilder()
                .setTitle("🔄 **CONVERT POINT BERHASIL**")
                .setColor(0x00ff88)
                .addFields(
                    { name: "Points digunakan", value: `${amount} points`, inline: true },
                    { name: "Credits didapat", value: `${creditsAmount.toLocaleString()} credits`, inline: true },
                    { name: "Rate", value: "1 point = 100 credits", inline: true }
                )
                .setTimestamp();
            
            return message.reply({ embeds: [embed] });
            
        } catch (error) {
            console.error("Convert point error:", error);
            return message.reply("❌ Terjadi kesalahan saat convert!");
        }
    }
};